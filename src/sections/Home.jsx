...
const Home = () => {
    ...
    const getImgStyle = (p, staggerOffset = 0, customEndMargin = 38) => {
        ...
        const maxW = isSmallMobile ? 90 : (isMobile ? 120 : 300);
        const maxWidth = `${maxW * (1 - lp)}px`;

        let startMargin = 30;
        let endMargin = customEndMargin;

        if (customEndMargin === 0) {
            endMargin = 0;
            startMargin = isSmallMobile ? 6 : (isMobile ? 10 : 30);
        } else {
            if (isSmallMobile) {
                startMargin = 8;
                endMargin = 8; // 자연스러운 한 글자 띄어쓰기 간격
            } else if (isMobile) {
                startMargin = 12;
                endMargin = 10; // 500px 등 모바일/태블릿에서 띄어쓰기 간격
            } else if (isTablet) {
                startMargin = 18;
                endMargin = 18;
            } else {
                startMargin = 30;
                endMargin = customEndMargin; // 데스크톱 38px
            }
        }
